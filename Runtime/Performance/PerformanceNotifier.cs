...
namespace Ouroboros.Common.Performance
{
    public class PerformanceNotifier : MonoBehaviour
    {
        ...
        public void Init()
        {
            ...
            {
                cond.tracker = PerformanceTracker.Instance.GetTracker(cond.trackerType) as StatsTracker;
            }
        }
        ...
        private string PrintTrackers()
        {
            ...
            {
                var statsTracker = tracker as StatsTracker;
                if (statsTracker != null)
                {
                    sb.AppendFormat("{0}: {1} (Min {2}, Max {3}, Avg {4}) ", statsTracker.Key, statsTracker.Current, statsTracker.Lowest, statsTracker.Highest, statsTracker.Avg);
                }
            }
